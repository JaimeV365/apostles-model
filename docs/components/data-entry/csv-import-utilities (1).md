...
export const findDataHeaders = (headers: string[]): { 
  ...
  for (const header of headers) {
    ...
    // Find loyalty header
    if ((normalized.includes('loy') || normalized.includes('nps')) && !loyaltyHeader) {
      loyaltyHeader = header;
      ...
      if (scale) {
        loyaltyScale = scale;
        if (!valid) {
          errors.push(`Invalid loyalty scale format in header "${header}". Valid scales are 1-5, 1-7, or 1-10.`);
        }
      } else {
        errors.push(`Missing scale in loyalty header "${header}". Format should be like "Loyalty:1-5" or "Loy-1-7".`);
      }
    }
  }
...
```
Finds and extracts data headers and their associated scales from CSV headers.
      ...
      try {
        ...
        for (const key of Object.keys(row)) {
          ...
          if (keyLower === 'date' || keyLower.includes('date')) {
            dateValue = row[key];
            if (keyLower.includes('(') && keyLower.includes(')')) {
              const formatMatch = keyLower.match(/\((.*?)\)/);
              if (formatMatch && formatMatch[1]) {
                dateFormat = formatMatch[1];
              }
            }
        ...
        } else if (loyalty < 1 || loyalty > Number(loyaltyScale.split('-')[1])) {
          rowErrors.push(`Invalid loyalty value ${loyalty} (should be between 1 and ${loyaltyScale.split('-')[1]})`);
        }
        ...
        if (date) {
          ...
          if (!parseResult.isValid) {
            ...
            if (parseResult.warning && rowErrors.length === 0) {
              warningRows.push({
                row: index + 2,
        ...
        // If errors, add to rejected rows
        if (rowErrors.length > 0) {
          rejectedRows.push({
            row: index + 2,
            id: processedRow.id || 'unknown',
            reason: rowErrors.join('; '),
            value: JSON.stringify(Object.entries(row)
              .filter(([key]) => ['ID', satisfactionHeader, loyaltyHeader, 'Date'].some(field => key.includes(field)))
              .map(([key, val]) => `${key}: ${val}`)
              .join(', '))
          });
          return null;
...
```
Validates and processes data rows from a CSV file, handling satisfaction, loyalty, date, and email validation.
